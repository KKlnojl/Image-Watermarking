...
class Ui:
    ...
    def show(self):
        self.img = self.img.resize((self.img_width, self.img_height))  # 將圖還原(初始化)，以利重新計算比例縮放顯示
        self.img_width = self.img.size[0]  # 將原始圖片大小儲存，瀏覽後最後要再轉回原本大小用
        self.img_height = self.img.size[1]  # 將原始圖片大小儲存，瀏覽後最後要再轉回原本大小用
        self.resize(w=self.img_width, h=self.img_height, w_box=self.field_img.winfo_width(),
                    h_box=self.field_img.winfo_height())
        print(f"show add rotate x: {self.img_width} rotate y: {self.img_height}")
    ...
    def add_watermark(self):
        ...
        self.canvas.itemconfig(self.canvas_img, image=self.show_img)
        self.img = self.img.resize((self.img_width, self.img_height))  # 預覽圖片
        self.img.show()
    ...
    def get_wm_position(self):
        ...
        # print(f"get wm position  rotate x: {self.img_width} rotate y: {self.img_height}")
